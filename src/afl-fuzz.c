...
u8 run_gen_seeds(afl_state_t * afl, u8 init_flag){
  ...
  if (init_flag == 1){
    ...
    printf("\n==== Gen-M start ====\n");
    snprintf(command, sizeof(command), "%s/generator_mutation_gpt_3_5_main --output %s >> %s/mutate_log", afl->generator_base_path, afl->out_dir, afl->out_dir);
  }
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (likely(!afl->stop_soon)) {
    ...
    if (get_cur_time() - afl->last_find_time > 60 * 1000 && get_cur_time() - gen_last_update > 300 * 1000){
